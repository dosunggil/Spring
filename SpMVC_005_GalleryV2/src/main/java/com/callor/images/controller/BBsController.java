...
public class BBsController {
	...
	/*
	 * @ModelAttribute("bbsVO22") BBsVO bbsVO
	 * Spring 아 내가 지금 BBsVO type 의 bbsVO 객체가 필요하니
	 * 나에게 주입(inject) 해달라 라는 요청
	 * 
	 * Spring 은 어딘가에 있는 @ModelAttribute("bbsVO22") 가 설정된
	 * method 를 찾아서 실행하여 return 값을 주입해준다 
	 */
	@RequestMapping(value="/write",method=RequestMethod.GET)
	public String write(@ModelAttribute("bbsVO22")  BBsVO bbsVO, Model model) {
		
		/*
		 * 주입받은 bbsVO 객체 데이터를 form(.jsp) 로 전달하기
		 */
		model.addAttribute("bbsVO3",bbsVO);
		return "bbs/input";
		
	}
	
	@RequestMapping(value="/write",method=RequestMethod.POST)
	public String write(@ModelAttribute("bbsVO22")  BBsVO bbsVO, @RequestParam("up_file")  MultipartFile file, Model model) {
...
//			bbsVO.setImages(fileDao.fineByBBsSeq(b_seq));
	...
	@ModelAttribute("bbsVO22")
	private BBsVO bbsVO1() {
