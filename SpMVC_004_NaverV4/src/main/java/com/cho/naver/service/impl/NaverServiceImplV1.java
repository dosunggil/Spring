...
import com.cho.naver.model.MovieVO;
...
public class NaverServiceImplV1 extends NaverServiceImpl{
	...
	// String cat와 queryString() 를 상속받는다.
	...
	public List<Object> getNaver(String queryString) {
		...
		if(cat.equals("BOOK")) {
			...
			resData = restTemp.exchange(restURI, HttpMethod.GET, entity, new ParameterizedTypeReference<NaverParent<BookVO>>() {} );
			return (List<Object>) resData.getBody().items;
		} else if (cat.equals("NEWS")) {
			ResponseEntity<NaverParent<NewsVO>> resData= null;
			resData = restTemp.exchange(restURI, HttpMethod.GET, entity, new ParameterizedTypeReference<NaverParent<NewsVO>>() {} );
			return (List<Object>) resData.getBody().items;
		} else if (cat.equals("MOVIE")) {
			ResponseEntity<NaverParent<MovieVO>> resData= null;
			resData = restTemp.exchange(restURI, HttpMethod.GET, entity, new ParameterizedTypeReference<NaverParent<MovieVO>>() {} );
			return (List<Object>) resData.getBody().items;
		}
